...
func newTestClient(t *testing.T, mux *http.ServeMux) *Client {
	...
	t.Cleanup(srv.Close)
	return NewClient(srv.URL, srv.URL+"/oauth/tokens", config.CatalogCredential{})
}
...
func TestCommitTransaction_Success204(t *testing.T) {
...
func TestCommitTransaction_Success200(t *testing.T) {
	// Some Iceberg REST implementations (Polaris, Nessie) return 200 OK instead of 204.
	mux := http.NewServeMux()
	mux.HandleFunc("/v1/namespaces/ns/tables/tbl/transactions/commit", func(w http.ResponseWriter, r *http.Request) {
...
func TestAuth_TokenRefreshedWhenExpired(t *testing.T) {
	...
	c := newTestClientWithAuth(t, mux)
	// Force token to appear already expired.
...
func TestMarshalTransaction_AddSnapshot(t *testing.T) {
	snap := iceberg.Snapshot{
		SnapshotID:     42,
		SequenceNumber: 3,
		TimestampMs:    1700000000000,
		ManifestList:   "s3://bucket/snap-42.avro",
	}
	snapRef := iceberg.Snapshot{SnapshotID: 42}
	tx := catalog.Transaction{
		Requirements: []catalog.Requirement{
			{Type: "assert-current-snapshot-id", CurrentSnapshotID: 10},
		},
		Updates: []catalog.Update{
			{Type: "add-snapshot", Snapshot: &snap},
			{Type: "set-snapshot-ref", RefName: "main", Snapshot: &snapRef},
			{Type: "remove-snapshots", SnapshotIDs: []int64{9, 10}},
		},
	}

	out := marshalTransaction(tx)
	...
	require.Len(t, decoded.Requirements, 1)
	assert.Equal(t, "assert-current-snapshot-id", decoded.Requirements[0]["type"])
	assert.EqualValues(t, 10, decoded.Requirements[0]["snapshot-id"])

	require.Len(t, decoded.Updates, 3)

	// add-snapshot must carry a snapshot object but NO top-level snapshot-id.
	addSnap := decoded.Updates[0]
	assert.Equal(t, "add-snapshot", addSnap["type"])
	assert.NotNil(t, addSnap["snapshot"])
	_, hasTopID := addSnap["snapshot-id"]
	assert.False(t, hasTopID, "add-snapshot must not carry a top-level snapshot-id")

	// set-snapshot-ref must carry a top-level snapshot-id (the ref target).
	setRef := decoded.Updates[1]
	assert.Equal(t, "set-snapshot-ref", setRef["type"])
	assert.Equal(t, "main", setRef["ref-name"])
	assert.EqualValues(t, 42, setRef["snapshot-id"])

	// remove-snapshots carries snapshot-ids array only.
	remove := decoded.Updates[2]
	assert.Equal(t, "remove-snapshots", remove["type"])
	assert.NotEmpty(t, remove["snapshot-ids"])
}
