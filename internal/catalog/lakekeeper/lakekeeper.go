...
func New(cfg config.CatalogConfig) *Client {
	...
	if cfg.Credential.ClientID != "" {
		c.tokens = newTokenSource(c.http, cfg.URL, cfg.Credential.ClientID, cfg.Credential.ClientSecret)
	}
	return c
}
...
// encodeNamespace converts a dot-separated namespace string into the Lakekeeper
// URL path segment, joining multipart namespaces with the ASCII unit separator.
...
func newTokenSource(hc *http.Client, baseURL, clientID, clientSecret string) *tokenSource {
	return &tokenSource{
		httpClient:   hc,
		tokenURL:     strings.TrimRight(baseURL, "/") + "/oauth/tokens",
		clientID:     clientID,
